CLASS lhc_ZI_DXTRAVEL_D DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zi_dxtravel_d RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR zi_dxtravel_d RESULT result.
    METHODS precheck_update FOR PRECHECK
      IMPORTING entities FOR UPDATE zi_dxtravel_d.

    METHODS accepttravel FOR MODIFY
      IMPORTING keys FOR ACTION zi_dxtravel_d~accepttravel RESULT result.

    METHODS deductdiscount FOR MODIFY
      IMPORTING keys FOR ACTION zi_dxtravel_d~deductdiscount RESULT result.

    METHODS recalctotalprice FOR MODIFY
      IMPORTING keys FOR ACTION zi_dxtravel_d~recalctotalprice.

    METHODS rejecttravel FOR MODIFY
      IMPORTING keys FOR ACTION zi_dxtravel_d~rejecttravel RESULT result.

    METHODS calctotprice FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zi_dxtravel_d~calctotprice.

    METHODS setstatusopen FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zi_dxtravel_d~setstatusopen.

    METHODS settravelid FOR DETERMINE ON SAVE
      IMPORTING keys FOR zi_dxtravel_d~settravelid.
    METHODS validatecustomer FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxtravel_d~validatecustomer.

ENDCLASS.

CLASS lhc_ZI_DXTRAVEL_D IMPLEMENTATION.
...
*    ENDLOOP.
    ...
    IF sy-subrc IS INITIAL.
      ...
      LOOP AT entities INTO DATA(ls_entity).
        ...
        IF sy-subrc IS NOT INITIAL.
          failed-zi_dxtravel_d = VALUE #( ( %tky = ls_entity-%tky ) ).
  ...
  METHOD deductDiscount.
    ...
    DATA  lt_travel_new TYPE TABLE FOR UPDATE Zi_DXtravel_d.
    DATA(lt_keys) = keys.
    ...
    LOOP AT lt_keys ASSIGNING FIELD-SYMBOL(<ls_keys>) WHERE %param-discount IS INITIAL
      ...
      APPEND VALUE #( %tky = <ls_keys>-%tky  )  TO  failed-Zi_DXtravel_d .
  ...
  METHOD reCalcTotalPrice.
    ...
    LOOP AT lt_travels ASSIGNING FIELD-SYMBOL(<travel>).
      ...
      READ ENTITIES OF Zi_DXtravel_d IN LOCAL MODE
        ENTITY zi_dxbooking__d BY \_BookingSupplement
  ...
  METHOD validateCustomer.
    ...
    LOOP AT lt_travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL.
        APPEND VALUE #( %tky = travel-%tky ) TO failed-zi_dxtravel_d.
