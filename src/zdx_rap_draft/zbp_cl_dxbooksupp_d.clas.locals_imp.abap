CLASS lhc_ZI_DXBOOKSUPL_D DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS calculateTotalPrice FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zi_dxbooksupl_d~calculateTotalPrice.

    METHODS setBookSupplNumber FOR DETERMINE ON SAVE
      IMPORTING keys FOR zi_dxbooksupl_d~setBookSupplNumber.
    METHODS validateSupplement FOR VALIDATE ON SAVE
      IMPORTING keys FOR ZI_DXBOOKSUPL_D~validateSupplement.

ENDCLASS.

CLASS lhc_ZI_DXBOOKSUPL_D IMPLEMENTATION.

  METHOD calculateTotalPrice.
    " Read all parent UUIDs
    READ ENTITIES OF ZI_DXTRAVEL_D IN LOCAL MODE
      ENTITY ZI_DXBOOKSUPL_D BY \_Travel
        FIELDS ( TravelUUID  )
  ...
  METHOD setBookSupplNumber.
    ...
    DATA bookingsupplements_update TYPE TABLE FOR UPDATE ZI_DXTRAVEL_D\\ZI_DXBOOKSUPL_D.
    ...
    READ ENTITIES OF ZI_DXTRAVEL_D IN LOCAL MODE
      ENTITY ZI_DXBOOKSUPL_D BY \_Booking
        FIELDS (  BookingUUID  )
        WITH CORRESPONDING #( keys )
  ...
  METHOD validateSupplement.
    ...
    LOOP AT bookingsupplements ASSIGNING FIELD-SYMBOL(<bookingsupplement>).
      ...
      APPEND VALUE #( %tky        = <bookingsupplement>-%tky
             ...
             TO reported-zi_dxbooksupl_d.

      IF <bookingsupplement>-SupplementID IS INITIAL.
        APPEND VALUE #( %tky = <bookingsupplement>-%tky ) TO failed-zi_dxbooksupl_d.
        ...
        APPEND VALUE #(
            ...
            %path                 = VALUE #(
                zi_dxbooking__d-%tky = booksuppl_booking_links[ KEY id
                                                             source-%tky = <bookingsupplement>-%tky ]-target-%tky
                zi_dxtravel_d-%tky  = booksuppl_travel_links[  KEY id
                                                            source-%tky = <bookingsupplement>-%tky ]-target-%tky )
