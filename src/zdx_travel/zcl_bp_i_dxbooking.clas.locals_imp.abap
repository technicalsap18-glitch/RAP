CLASS lhc_zi_dxbooking DEFINITION INHERITING FROM cl_abap_behavior_handler.

  PRIVATE SECTION.

    METHODS earlynumbering_cba_Booksupp FOR NUMBERING
      IMPORTING entities FOR CREATE zi_dxbooking\_Booksupp.
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zi_dxbooking RESULT result.
    METHODS validateconnection FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxbooking~validateconnection.

    METHODS validatecurrencycode FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxbooking~validatecurrencycode.

    METHODS validatecustomer FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxbooking~validatecustomer.

    METHODS validateflightprice FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxbooking~validateflightprice.

    METHODS validatestatus FOR VALIDATE ON SAVE
      IMPORTING keys FOR zi_dxbooking~validatestatus.
    METHODS caltotalprice FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zi_dxbooking~caltotalprice.

ENDCLASS.

CLASS lhc_zi_dxbooking IMPLEMENTATION.
     ...
     GROUP BY <booking_group>-%tky.
      ...
      LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking>) USING KEY entity WHERE TravelId  = <booking_group>-TravelId
        ...
        LOOP AT <booking>-%target ASSIGNING FIELD-SYMBOL(<booksuppl_wo_numbers>).
          APPEND CORRESPONDING #( <booksuppl_wo_numbers> ) TO mapped-zdx_booksupp ASSIGNING FIELD-SYMBOL(<mapped_booksuppl>).
  ...
  METHOD calTotalPrice.
    ...
    MODIFY ENTITIES OF zi_dxtravel  IN LOCAL MODE
          ENTITY zi_dxtravel
          EXECUTE reCalPrice FROM CORRESPONDING #( it_travel ).
  ENDMETHOD.
